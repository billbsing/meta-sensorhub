...
SRC_URI = "git://git@github.com/newtoncircus/silverline-sensor-hub.git;branch=test-1.2;protocol=ssh;tag=v${PV} \
	    ...
	    file://shdapAPI-server.service \
	    file://sensorhub-bluetooth-rfkill.conf \
"
...
do_install () {
    ...
    install -m 0644 ${WORKDIR}/shdapAPI-server.service ${D}/${systemd_unitdir}/system/

    install -d ${D}${systemd_unitdir}/system/${PN}.bluetooth.d
    install -m 0644 ${WORKDIR}/sensorhub-bluetooth-rfkill.conf ${D}${systemd_unitdir}/system/${PN}.bluetooth.d
...
pkg_postinst_${PN} ()  {
	...
	systemctl stop shdapAPI-server
	systemctl stop lighttpd 
	/opt/sensorhub/tools/checkSystemFiles.lua --write
	systemctl start lighttpd 
	systemctl start shdapAPI-server
