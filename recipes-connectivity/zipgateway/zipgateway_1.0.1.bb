...
do_compile() {
	cd ${S}/
        patch -p0 < zipgateway.patch
	#apply this patch to disable building udprelay
	patch -p0 < zipgw_disble_pcap.patch
	#apply patch to change the libusb path
	patch -p0 < libusb_path.patch
        mkdir -p ${S}/${PN}/usr/local/build
	cd ${S}/${PN}/usr/local/build
	cmake -DSYSROOT_PATH=${STAGING_DIR_TARGET} ..
	${MAKE}
}
do_install() {
	install -d ${D}${bindir}
	...
	cp ${S}/${PN}/usr/local/main.conf ${D}/${sysconfdir}/connman/
	...
	install -m 0755 ${S}/${PN}/usr/local/build/zipgateway ${D}${bindir}
	install -m 0755 zipgateway.tun ${D}${sysconfdir}/zipgateway
	install -m 0755 ${S}/${PN}/usr/local/WRTpackage/files/*.pem ${D}${sysconfdir}/zipgateway
	install -m 0755 zipgateway.cfg ${D}${sysconfdir}/zipgateway

	install -d ${D}/${systemd_unitdir}/system
	install -m 0644 ${S}/zipgateway.service ${D}${systemd_unitdir}/system/

	install -m 0755 ${S}/zipgateway-start ${D}${bindir}
	install -m 0755 ${S}/zipgateway-stop ${D}${bindir}
}
